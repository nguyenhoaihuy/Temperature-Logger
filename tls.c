...
int main(int argc, char* argv[]){
	...
	newContext = SSL_CTX_new(sslMethod);
	if (newContext == NULL){
	...
	if (SSL_set_fd(sslClient, client_fd) == 0){	
		fprintf(stderr, "ERROR: Unable to redirect client fd\n");
		exit(1);
	}
	if (SSL_connect(sslClient) != 1){
		fprintf(stderr, "ERROR: Unable to make SSL handshake\n");
		exit(1);
	}
	...
	char id_str[15];
	sprintf(id_str, "ID=%s\n", id_number);
	if (SSL_write(sslClient, &id_str,strlen(id_str))<0){
	...
	while (run_flag){
		...
		if (pfds[0].revents & POLLIN){
			int n = SSL_read(sslClient, buf, MAX_SIZE);
			if (n<0){
		...
		if (stop != 1){
			...
			if (SSL_write(sslClient, result, strlen(result))<0){
...
void do_when_interrupted(){
	...
	run_flag = 0;
	char shutdown_str[30];
	sprintf(shutdown_str,"%02d:%02d:%02d SHUTDOWN\n", tm->tm_hour, tm->tm_min, tm->tm_sec);
	if (SSL_write(sslClient, shutdown_str, strlen(shutdown_str))<0){
		fprintf(stderr, "ERROR: Cannot write shutdown to the server\n");
		exit(1);
	}
        if (log_opt==1)
